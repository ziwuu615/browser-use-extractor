...
Computer Use 方式本身具备天然的反爬优势：控制真实浏览器（Chrome/Edge）、带真实指纹、执行 JS，大量针对「脚本请求」的前端反爬对它无效。剩余的反爬分层处理：

- **并发限流**（已做）：`MAX_CONCURRENCY` 信号量限制并发浏览器实例，避免高频触发 IP 封禁。
- **IP 代理池**（待做）：单 IP 高频访问仍可能被限流，接代理池轮换。
- **行为拟人化**（待做）：随机操作节奏、模拟鼠标轨迹，降低行为分析识别的风险。
- **验证码边界**：视觉模式（Qwen-VL）可处理识别型验证码；滑块 / reCAPTCHA 基本无解，选择承认边界、换数据源而非硬破。
